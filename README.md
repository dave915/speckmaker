...
상단의 **AI 수정**에서 `http://localhost:11434`에 연결하고 설치된 모델을 선택합니다. 수정 요청과 현재 HTML은 브라우저에서 Ollama의 `/api/chat`으로 직접 전송되며, 서버 중계는 사용하지 않습니다. 모델 목록은 `/api/tags`에서 불러옵니다. 스트리밍 생성, 중단, 시간 초과, 미완성 HTML 차단을 지원합니다. AI 수정용 입력은 48KB까지 지원합니다(일반 HTML 업로드는 5MB).

수정안은 먼저 캔버스에서 원본과 비교할 수 있으며 **적용하고 이력 저장**을 눌러야 현재 목업이 바뀝니다. 다른 사용자가 먼저 수정하면 이전 내용을 덮어쓰지 않고 충돌을 안내합니다.

배포된 사이트에서 로컬 Ollama에 접근하려면 Ollama에 사이트 출처를 허용해야 합니다. 이미 실행 중인 Ollama는 종료한 뒤 실행합니다.

```sh
OLLAMA_ORIGINS="https://speckmaker-review.helloooo33.chatgpt.site,http://localhost:5173,http://127.0.0.1:5173" ollama serve
```

Chrome 등에서 로컬 네트워크 접근 권한을 요청하면 허용해야 합니다. 모델과 주소는 기기별 설정으로만 저장합니다. API 키를 사이트 서버에 저장하거나 요청을 프록시하지 않습니다. macOS 앱 실행 방식의 환경 변수 설정은 [Ollama 공식 FAQ](https://docs.ollama.com/faq#how-can-i-allow-additional-web-origins-to-access-ollama)를 참고하세요.
